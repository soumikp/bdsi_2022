...
At the end of this review, you will be able to 
  ...
  1. Recall basic notions of probability (through counting-based examples).
      - Coding skill: recall the basic framework for estimating probability through simulated repetitions of an experiment. 
  
  2. Understand notions of conditional probability.
      - Coding skill: implement control structures by introducing the use of `if` statements for counting successes and for simulating populations based on conditional probabilities.
      
  3. Link random variables to concepts of probability. 
      - Coding skill: generate random samples from a given distribution and use Monte Carlo methods to estimate distribution parameters.
...
# 1A: Basic concepts of probability: Introduction 
...
# 1B: Basic concepts of probability: Random experiments
...
# 1C: Basic concepts of probability: Formalising our notion of probability (somewhat)
...
# 1D: Basic concepts of probability: Back to events and outcomes!
...
# 1E: Basic concepts of probability:  Addition rule for disjoint outcomes and inclusion-exclusion principle.
...
where the ‘cup’ $\cup$ symbol denotes the union of two events and the ‘cap’ $\cap$ symbol denotes the intersection of two events.
</center>

# 1F: Basic concepts of probability: Complement of an event.
...
# 1H: Basic concepts of probability: Independence
<center>
...
Notes for review may be found [here](https://rpubs.com/soumikp/bdsi22ProbNotes).
...
# 2A: Conditional probability: Intuition
...
>- The conditional probability of an event $A$, given a second event $B$, is the probability of $B$ happening, knowing that the event $B$ has happened. Denoted by `r colourise("$P(A|B)$", "red")`.
...
# 2B: Conditional probability: Formal defintion

We must ensure the event we are conditioning on is not an impossible event, i.e., $P(B) \neq 0$. We define conditional probability of $A$ conditioned on $B$ as follows: 

$$P(A|B) = \frac{P(A \cap B)}{P(B)}.$$

<hr>

Link with independence: if $P(A|B) = P(A)$, then $A$ and $B$ are independent. 

`r colourise("Knowing whether $B$ happened or not offers no information about whether $A$ occurred.", "red")`
...
>- For clinicians, the question above may be reformulated: `r colourise("what is the conditional probability that a woman has breast cancer, given that her mammogram is abnormal?", "red")` This conditional probability is called the **positive predictive value (PPV)** of a mammogram. More concisely, if 
...
>- The characteristics of a mammogram (and other diagnostic tests) are given with the reverse conditional probabilities — the probability that the mammogram correctly returns a positive result if a woman has breast cancer (i.e., $P(B|A)$), as well as the probability that the mammogram correctly returns a negative result if a woman does not have breast cancer (i.e., $P(B^C|A^C)$). 
...
>- There are several possible strategies for approaching this type of problem; we will focus on `r colourise("using a purely algebraic approach using Bayes’ Theorem.", "blue")`

# Lab 2C: Bayes' theorem through an example: Bayes' theorem statement
...
# Lab 2D: Bayes' theorem through an example: Defining events in diagnostic testing
...
```{r, echo=FALSE}
   knitr::include_graphics('./figures/ppv formula.jpeg')
```

# Lab 2E: Bayes' theorem through an example: Calculating PPV using Bayes' theorem
...
# 3B: Distributions of random variables: Distribution of a discrete random variable
...
# 3C: Distributions of random variables: Expectation and variance
...
# 3D: Distributions of random variables: Binomial random variables
...
# 3E: Distributions of random variables: Continuous random variables 
...
# 3F: Distributions of random variables: Probability density functions
...
# 3G: Distributions of random variables: Normal distribution
...
# 3H: Distributions of random variables: Calculating normal probabilities using `R`
